...
更多关于“运行时生成不同容量模板”的算法与接口（默认会自动收敛到按对齐约束推导出的最小模板，再按需扩容），见 `doc/TEMPLATE_ARRAY_GENERATION.md`。
如果你要离线重生更小的 embedded arrays（并裁剪 PDB 模板尾部 slack），可使用 `tools/jit_template_minimizer.py`。
